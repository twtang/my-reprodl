...
def train(cfg: DictConfig):
    ...
    train_data = ESC50Dataset(
        path=path, sample_rate=cfg.data.sample_rate, folds=cfg.data.train_folds
    )
    val_data = ESC50Dataset(
        path=path, sample_rate=cfg.data.sample_rate, folds=cfg.data.val_folds
    )
    test_data = ESC50Dataset(
        path=path, sample_rate=cfg.data.sample_rate, folds=cfg.data.test_folds
    )
